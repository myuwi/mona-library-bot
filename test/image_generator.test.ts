...
it("should generate image with background", async () => {
  ...
  const buffer = await team(data, { background: true });
...
it("should generate image without background", async () => {
  const data = (
    ["Kamisato Ayaka", "Mona", "Kaedehara Kazuha", "Diona"] as const
  ).map((c) => getCharacterByName(c));

  const buffer = await team(data, { background: false });
  expect(buffer).toBeInstanceOf(Buffer);
