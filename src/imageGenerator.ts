...
// TODO: replace with zod schema parser
export function checkOptions(options: GeneratorOptions = {}) {
  ...
  if (opts.size > 10) {
    throw new OptionsError(
      "Team size cannot exceed the maximum allowed size (10)"
    );
  }
...
export async function team(
  ...
  const comps = getComps(data, opts);
