    ...
    if (command.permissionLevel !== PermissionLevel.BOT_OWNER) {
        ...
        if (userOverrides.length) {
            const override = userOverrides.find((o) => o.commandName === command.name)
                || userOverrides.find((o) => o.commandName === '*');
        ...
        const roleOverrides = (await client.db.guilds.permissions.roles.get(member.guild.id))
            .filter((o) => o.commandName === '*' || o.commandName === command.name);
        ...
        for (let i = 0; i < memberRoles.length; i++) {
            ...
            const override = roleOverrides.find((o) => o.roleId === memberRole.id && o.commandName === command.name)
    ...
    switch (command.permissionLevel) {
        ...
        case PermissionLevel.ADMIN:
            return member.id === member.guild.ownerId
                || member.permissions.any(['MANAGE_GUILD'], true);
        case PermissionLevel.MODERATOR:
            return member.id === member.guild.ownerId
                || member.permissions.any(['MANAGE_GUILD', 'MANAGE_CHANNELS'], true);
